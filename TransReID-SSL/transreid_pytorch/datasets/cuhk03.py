            ...
            for i, (campid, pid, img_paths) in enumerate(meta_data):
                ...
                if [campid, pid] in test_split:
                    for img_path in img_paths:
                        campid = int(
                            osp.basename(img_path).split('_')[0]
                        )
                        viewid = int(
                            osp.basename(img_path).split('_')[2]
                        )
                        camid = (campid - 1) * 2 + viewid
                        # camid = int(
                        #     osp.basename(img_path).split('_')[2]
                        # ) - 1 # make it 0-based
                        test.append((img_path, self.pid_begin + num_test_pids, camid, 1))
                    num_test_pids += 1
                    ...
                    for img_path in img_paths:
                        ...
                        #     osp.basename(img_path).split('_')[2]
                        # ) - 1 # make it 0-based
                        train.append((img_path, self.pid_begin + num_train_pids, camid, 1))
                    num_train_pids += 1
