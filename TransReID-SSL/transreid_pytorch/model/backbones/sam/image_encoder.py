...
class ImageEncoderViT(nn.Module):
    ...
    def load_param(self, model_path,hw_ratio):# try latest pytorch to run transreid
        ...
        for k, v in param_dict.items():
            ...
            elif (k.endswith("rel_pos_h") or k.endswith("rel_pos_w")) and v.shape != self.state_dict()[k].shape:
                v = resize_rel_pos(v, self.state_dict()[k])
            try:
...
def resize_rel_pos(rel_pos, rel_pos_new):
    rel_pos = rel_pos.unsqueeze(0).permute(0,2,1)
    rel_pos = F.interpolate(rel_pos, size=rel_pos_new.shape[0]).squeeze()
    rel_pos = rel_pos.permute(1, 0)
    return rel_pos
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        stride_size_tuple = to_2tuple(stride)

        self.num_x = (img_size[1] - kernel_size[1]) // stride_size_tuple[1] + 1
        self.num_y = (img_size[0] - kernel_size[0]) // stride_size_tuple[0] + 1
        print('using stride: {}, and patch number is num_y{} * num_x{}'.format(stride, self.num_y, self.num_x))
        ...
        if self.stem_conv:
            hidden_dim = 64
            stem_stride = 2
            stride_size = patch_size = kernel_size[0] // stem_stride
...
def vit_large_patch16_1024_sam(
        img_size=(1024, 1024),
        stride_size=16,
        drop_path_rate=0.1,
        camera=0,
