...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + self.num_tokens, embed_dim))#use sin cos position
    ...
    def prepare_tokens_with_masks(self, x, camera_id, view_id, masks=None):
        B, nc, w, h = x.shape
        x = self.patch_embed(x)
        if masks is not None: #mask can be freezed
    ...
    def forward_features(self, x, masks=None):
        ...
        x = self.prepare_tokens_with_masks(x, camera_id, view_id, masks)
...
def vit_small_patch14_224_dinov2(
        img_size=(224, 224),
        stride_size=14,
        drop_path_rate=0.1,
        camera=0,
...
def vit_base_patch14_224_dinov2(
        img_size=(224, 224),
        stride_size=14,
        drop_path_rate=0.1,
        camera=0,
