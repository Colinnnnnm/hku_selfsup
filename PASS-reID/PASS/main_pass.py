...
def train_dino(args):
    ...
    # ============ preparing data ... ============
    transform = DataAugmentationDINO( # check how to pre-process data and learning strategy
        (args.height,args.width),
        (args.crop_height,args.crop_width), # try to obey the same hyper-parameters
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
    ...
    for it, (images, _) in enumerate(metric_logger.log_every(data_loader, 200, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            loss = dino_loss(teacher_output_g, student_output_g, student_output_pt1, student_output_pt2, student_output_pt3, epoch) # try to find out what is dino_loss, try to remove teacher
